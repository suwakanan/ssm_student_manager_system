								...
								<c:forEach items="${selectedCourseList}" var="item">
									<tr>
										...
										<c:if test="${item.over}">
											<td>${item.mark}</td>
											<td>已打分</td>
											<%-- 以下是2020.05.27新增内容，作用是可以修改分数，原本不可以修改 --%>
											<td>
												<button class="btn btn-default btn-xs btn-info" onClick="location.href='/teacher/mark?studentid=${item.studentid}&courseid=${item.courseid}'">修改</button>
											</td>
											<%-- 以上是2020.05.27新增内容，作用是可以修改分数，原本不可以修改 --%>
										</c:if>
									</tr>
								</c:forEach>
					        </tbody>
